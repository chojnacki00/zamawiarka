      ...
      <div v-if="!isFormOpen">
        ...
        <div style="position: sticky; top: 0; z-index: 10; background: #f9fafb; padding: 20px 20px 10px 20px; border-bottom: 1px solid #e5e7eb;">
          ...
          <div style="position: relative; width: 100%;">
            ...
            <!-- Przycisk X (Pojawia się tylko gdy coś jest wpisane) -->
            <button 
        ...
        <div style="padding: 10px 20px 20px 20px;">
          ...
          <div v-else>
            <div 
              ...
              :key="pracownik.id"
              class="item-card"
              style="padding: 15px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; gap: 15px;"
            >
              <div style="flex: 1; min-width: 0;">
                ...
                <div style="font-size: 12px; color: #6b7280; display: flex; gap: 8px; align-items: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                  ...
                  <span style="color: #9ca3af; font-weight: 600;">PIN: ****</span>
                </div>
              </div>
      ...
      <div v-else style="padding: 20px;">
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 20px;">
          <div style="flex: 1;">
            ...
            <input 
              ...
              style="width: 100%; padding: 12px 15px; border: 1px solid #d1d5db; border-radius: 10px; font-size: 16px; box-sizing: border-box; outline: none;"
            />
          </div>
          <div style="flex: 1;">
            <label style="display: block; font-size: 13px; font-weight: 600; color: #6b7280; text-transform: uppercase; margin-bottom: 6px;">
        ...
        <div style="margin-bottom: 20px;">
          ...
          >
            ...
            <option v-for="rola in rolesStore.roles" :key="rola.id" :value="rola.id" style="color: #111827;">
              {{ rola.nazwa }}
            </option>
          </select>
        </div>
        ...
        <div style="margin-bottom: 20px;">
          ...
          <div style="display: flex; gap: 8px; width: 100%;">
            <input 
              ...
              maxlength="4"
              class="form-input"
              style="flex: 1; min-width: 0; padding: 12px 15px; border: 1px solid #d1d5db; border-radius: 10px; font-size: 18px; font-weight: 600; letter-spacing: 2px; text-align: center; box-sizing: border-box; outline: none;"
            />
            <button 
              @click="generateRandomPin"
              style="white-space: nowrap; flex-shrink: 0; padding: 0 15px; background: #e0f2fe; color: #0284c7; border: 1px solid #bae6fd; border-radius: 10px; font-weight: 600; cursor: pointer;"
            >
              🎲 Losuj
            </button>
          </div>
        ...
        <h3 style="font-size: 13px; font-weight: 600; color: #6b7280; text-transform: uppercase; margin: 30px 0 15px 0; border-bottom: 1px solid #e5e7eb; padding-bottom: 8px;">
          Dane dodatkowe (opcjonalnie)
        </h3>
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 15px;">
          <div style="flex: 1;">
            ...
            <input 
              ...
              class="form-input"
              style="width: 100%; padding: 12px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 15px; box-sizing: border-box; outline: none;"
            />
          </div>
          <div style="flex: 1;">
            <label style="display: block; font-size: 12px; font-weight: 600; color: #6b7280; margin-bottom: 6px;">
        ...
        <div style="display: flex; gap: 12px; margin-top: 30px; padding-bottom: 40px;">
          ...
          <button 
            ...
            :style="{ opacity: (!isFormValid || isSaving) ? 0.5 : 1 }"
          >
    ...
    <div v-if="showDeleteModal" style="position: fixed; inset: 0; background: rgba(0,0,0,0.6); display: flex; align-items: center; justify-content: center; z-index: 1000; padding: 20px;">
      <div style="background: white; border-radius: 20px; padding: 30px 20px; width: 100%; max-width: 340px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1); text-align: center;">
        ...
        <p style="color: #6b7280; font-size: 15px; margin-bottom: 25px; line-height: 1.5;">
          Czy usunąć pracownika: <br>
          <strong translate="no" class="notranslate" style="color: #374151; font-size: 16px;">{{ empToDelete?.imie }} {{ empToDelete?.nazwisko }}</strong>?
        </p>
...
const openForm = (emp = null) => {
  ...
  if (emp) {
    ...
    form.value = { imie: '', nazwisko: '', roleId: '', pin: '', stawka: '', telefon: '', email: '' }
    generateRandomPin()
  }
...
<style scoped>
/* Unikalna klasa dla wszystkich pól w formularzu */
.form-input {
  background-color: #f3f4f6 !important; /* Jasnoszary kolor tła */
  color: #0284c7 !important;            /* Niebieski tekst */
  font-weight: 600;
}

/* Stylowanie podpowiedzi we wszystkich polach formularza */
.form-input::placeholder {
  color: #111827 !important;            /* Czarny tekst */
  font-style: italic !important;        /* Kursywa */
  font-weight: 400 !important;
}

/* Wymuszenie czarnego koloru i kursywy również w pierwszej (nieaktywnej) opcji Selecta */
.placeholder-option {
