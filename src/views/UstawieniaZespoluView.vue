    ...
    <div ref="scrollAreaRef" class="scroll-area team-scroll">
      <template v-if="!isFormOpen">
        <p
          ...
          class="temporary-cleanup-warning"
          role="status"
        >{{ temporaryCleanupWarning }}</p>
...
onMounted(async () => {
  ...
  void accountSessionStore.cleanupCurrentRestaurantTemporaryData()
    .then(result => {
      const failures = getCleanupFailureDetails(result)
      if (!failures.length) return

      temporaryCleanupWarning.value =
        'Nie udało się usunąć części wygasłych danych. Możesz nadal pracować; spróbujemy ponownie później.'
      console.warn('Nie wszystkie dane tymczasowe zostały wyczyszczone.')
      if (import.meta.env.DEV) {
        console.warn('Szczegóły czyszczenia danych tymczasowych:', failures)
      }
    })
    .catch(error => {
      temporaryCleanupWarning.value =
        'Nie udało się sprawdzić wygasłych danych. Możesz nadal pracować; spróbujemy ponownie później.'
      console.warn('Nie udało się uruchomić czyszczenia danych tymczasowych.')
      if (import.meta.env.DEV) {
        console.warn('Szczegóły uruchomienia czyszczenia:', {
          operation: 'temporaryDataCleanup',
          collection: 'multiple',
...
const generatePairingCode = async () => {
  ...
  try {
    ...
    if (!cleanupResult.completed) {
      console.warn('Nie udało się wyczyścić wygasłych kodów parowania.')
      if (import.meta.env.DEV) {
        console.warn(
          'Szczegóły czyszczenia kodów parowania:',
          getCleanupFailureDetails({ pairingCodes: cleanupResult })
        )
      }
    }
