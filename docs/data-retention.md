...
| `restaurants/{restaurantId}/members/{authUid}/deviceSessions/{authTime}` | Zatwierdzona sesja urządzenia | Tak do odłączenia; potem techniczna historia | Aktywna do odłączenia; odłączona przez 90 dni | Manager odłącza jedną lub wszystkie sesje; sprzątanie usuwa odłączone dokumenty starsze niż 90 dni | Uprawniony manager uruchamia sprzątanie; konto Auth i pracownik pozostają | Wdrożone w tym etapie |
| `localStorage: gm_local_pin_v2:{authUid}:{deviceId}` | Osobna sól, weryfikator PBKDF2, licznik prób i czas blokady lokalnego PIN-u urządzenia | Nie, dane urządzenia | Do wylogowania, odłączenia urządzenia albo wyczyszczenia danych witryny | `logoutCurrentDevice` i wykrycie odłączenia usuwają rekord urządzenia | Ręczne wyczyszczenie danych witryny wymaga ponownego zaproszenia urządzenia | Wdrożone; jawny PIN nie jest zapisywany ani wysyłany |
...
- Błąd kolejnej partii zwraca wynik częściowy i nie blokuje otwarcia formularza
  ...
  czyszczenie.
- Odłączone sesje są usuwane po 90 dniach. Operacje nie mają ścieżki do
  grafików, dyspozycji, kont Auth ani dokumentów pracowników.
...
## Koszt kontroli urządzenia w regułach

Każdy chroniony odczyt lub zapis pracownika sprawdza dokument członkostwa,
sesję `deviceSessions/{auth_time}` i dane pracownika; operacje zależne od
uprawnień odczytują również profil uprawnień. Firestore może buforować te same
odczyty w ramach jednej oceny reguł, ale zależne odczyty reguł podlegają limitom
i mogą być rozliczane. Właściciel nadal omija wyłącznie kontrolę urządzenia,
nie kontrolę uwierzytelnienia i aktywnego członkostwa.
